...
// empty, the result is undetermined: an empty book and a missing book are
// different facts, and zero would claim a measurement where none exists.
func (m SpreadMetric) Run(ctx context.Context, s Subject) MetricResult {
	...
	// No bids or no asks means no two-sided market to measure a spread
	// against. This is the absence of a spread, not a zero spread.
