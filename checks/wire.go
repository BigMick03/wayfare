...
//
// Value is a decimal string, not a JSON number, so a client cannot parse it
// into a float64 and reintroduce rounding.
...
type FindingsJSON struct {
	...
	// Metrics are measured quantities, reported alongside checks but
	// never blended into them. A spread is a spread whether or not
	// the issuer's auth flags pass.
